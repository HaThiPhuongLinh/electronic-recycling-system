...
const Checkout = () => {
    ...
    const handleCheckout = async () => {
        ...
        try {
            ...
            navigate(`/sell/product/${params.productId}/checkout/shipment`, {
                state: {
    ...
    useEffect(() => {
        ...
        const fetchConditions = async () => {
            try {
                const conditions = await Promise.all(
                    selectedConditions.map(id => quotingAPI.getConditionById(id))
                );
                setConditionNames(conditions.map(condition => condition.data.name));
            } catch (error) {
        ...
        fetchProduct();
        fetchConditions();
    }, [params.productId, selectedConditions]);
