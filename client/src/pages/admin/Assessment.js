...
const Assessment = () => {
  ...
  const fetchData = async () => {
    try {
      const response = await receivedAPI.getACCEPTEDItems("PENDING");
      ...
      for (const item of items) {
        ...
        const internalEvaluatingStatus = statusData.state.find(s => s.status === "INTERNAL_EVALUATING");
  ...
  return (
    <div class="container px-4 mx-auto mt-4">
      <div class="max-w-lg mx-auto text-center mt-5">
        <h1 class="text-4xl font-bold text-black mb-4">Internal Assessment</h1>
      </div>
        ...
        <table className="min-w-full bg-white border">
          <thead className="sticky top-0 bg-white shadow">
            <tr>
              ...
              <th className="py-2 px-4 text-left text-gray-500 border">External Review Date</th>
          ...
          <tbody>
            {currentData.map((item, index) => (
              <tr key={item.quotingItemId} className={`border ${highlightedOrder === item.quotingItemId ? 'bg-green-200' : ''}`}>
                ...
                <td className="py-2 px-4 border">{item.internalEvaluatingStatus}</td>
      ...
      {isModalOpen && <AssessmentModal data={selectedItem} closeModal={closeModal} />}
    </div>
  );
};
